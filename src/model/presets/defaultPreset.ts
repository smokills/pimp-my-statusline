// defaultConfig() — faithful to the user's ~/.claude/statusline.sh (minus the
// retired peak-hours indicator, which Anthropic removed).
//
// Three lines:
//   ROW1: directory (ansi16 1;34) + "  " + gitBranch (ansi16 1;32)
//   ROW2: 74× '─' dim white (2;37)
//   ROW3: MODEL_PART CTX%  SESSION  WEEK
//         where MODEL_PART = model(1;37) ' ' effort(2;37)
//         and "MODEL_PART ${CTX}%" join is a SINGLE space, while the major
//         groups (…CTX%  SESSION  WEEK) join with TWO spaces.
//
...
//   model [ ' ' ] effort [ ' ' ] context [ '  ' ] session [ '  ' ] week
// which reproduces `${MODEL} ${EFFORT} ${CTX}%  ${SESSION}  ${WEEK}`.
...
export function defaultConfig(): StatuslineConfig {
  ...
  // ----- Row 3: model effort context  session  week -----
  ...
  return {
    ...
    rows: [
      ...
      {
        id: 'row3',
        segments: [model, effort, context, session, week],
        joiner: '  ',
      },
    ],
    pet: {
